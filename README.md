...
![Security Audit Demo](assets/audit_demo.svg)
...
```mermaid
graph TD
    subgraph Core ["Security & Hardening Core"]
        Audit["🐍 audit.py (Python)"]
        Scanner["🔍 local_security_audit.py"]
    end

    subgraph Engines ["High-Performance Engines"]
        C_Engine["⚙️ EventHive-C (Pure C)"]
        Go_Monitor["🐹 Health Monitor (Go)"]
    end
